...
def save_sent_emails_as_eml(server, username, password, output_folder, mailbox='"Éléments envoyés"', max_emails=None):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python mailtool_sent.py <server> <username> <password> <output_folder> [mailbox] [max_emails]")
        print("Exemples de boîtes pour les emails envoyés :")
        print("  - SENT (standard)")
        print("  - [Gmail]/Sent Mail (pour Gmail)")
        print("  - Envoyés (pour certains serveurs francophones)")
        sys.exit(1)
    ...
    mailbox = sys.argv[5] if len(sys.argv) > 5 else 'SENT'
    max_emails = int(sys.argv[6]) if len(sys.argv) > 6 else None

    save_sent_emails_as_eml(server, username, password, output_folder, mailbox, max_emails)